...
function PreviewCard({ src, label, placeholder }) {
  ...
  return (
    <div>
      <div className="flex h-40 items-center justify-center rounded-2xl bg-[#f3f1ea] text-center text-sm text-slate-400 ring-1 ring-slate-200">
        {isPlaceholder ? (
          <div>
            <div className="text-base font-medium text-slate-400">{placeholder}</div>
          </div>
        ) : (
  ...
  return (
    <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6">
      ...
      <form className="mt-4 grid gap-3" onSubmit={submit}>
        <label
          className="flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-300 bg-[#faf8f2] px-4 py-6 text-center transition hover:border-[#2D6A4F]"
        ...
        >
          ...
          <button
            className="mt-4 rounded-full border border-[#2D6A4F] bg-white px-4 py-2 text-sm font-semibold text-[#2D6A4F]"
            type="button"
      ...
      {error ? <div className="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-800">{error}</div> : null}
        ...
        <div className="mt-5 border-t border-slate-200 pt-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-[#f7f5ef] px-4 py-5 text-center">
              ...
              <div className="mt-1 text-3xl font-bold text-[#2D6A4F]">{result.total_tree_count}</div>
            </div>
            <div className="rounded-2xl bg-[#f7f5ef] px-4 py-5 text-center">
              ...
              <ul className="mt-3 divide-y divide-slate-200 rounded-2xl bg-white">
                {result.observations.map((item) => (
                  <li key={item} className="flex gap-3 px-2 py-3 text-sm text-slate-700">
                    <span className="mt-1 h-2 w-2 rounded-full bg-[#2D6A4F]" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="mt-5">
              ...
              <div className="mt-3 grid gap-3">
                {result.recommendations.map((item) => (
                  <div key={item} className="rounded-2xl border-l-4 border-[#2D6A4F] bg-[#f7fbf7] px-4 py-3 text-sm text-slate-700">
                    ✓ {item}
                  </div>
                ))}
              </div>
